...
public class register extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViews();
		setListener();
		text_view.setText(Data.test2[0]+"!!"+Data.test3[0]+Data.test2[1]+"!!"+Data.test3[1]);
	}
	...
	private OnClickListener submitAll = new OnClickListener() {
		public void onClick(View v) {
			...
			/*
			nw.disable_policy();
			nw.setUDPSend(nw.concat(nw.str2bytes(Data.keyWord),nw.str2bytes(username)));
			
			text_view.setText("2");
			byte[] rec_data=nw.setUDPReceive();
			
			System.arraycopy(rec_data,0,c,0,4);
			System.arraycopy(rec_data,4,d,0,16);
			System.arraycopy(rec_data,20,f,0,108);
			errorCode=nw.byteToInt(c);
			password=nw.bytes2str(d);
			errorString=nw.bytes2str(f);
			
			text_view.setText(Data.keyWord.length()+"first:"+errorCode+"..."+password+"..."+errorString);*/
